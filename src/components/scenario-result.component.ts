...
@Component({
    ...
    <div class="w-full max-w-6xl mx-auto px-4 py-6 min-h-screen">
      ...
      @if (loading()) {
        ...
        <div class="flex flex-col items-center justify-center h-64 text-slate-500">
          <p class="text-xl font-bold">رکورد یافت نشد</p>
          <button (click)="goBack()" class="mt-4 text-orange-600 font-bold hover:underline">بازگشت</button>
        </div>
      } @else {
        <div class="animate-fade-in pb-20">
          ...
          <div class="sticky top-4 z-40 bg-white/90 backdrop-blur-md p-2 rounded-xl shadow-sm border border-slate-200 mb-6 flex items-center justify-between no-print max-w-4xl mx-auto">
             ...
             <div class="flex gap-2">
                <button (click)="exportToWord()" class="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-lg font-bold hover:bg-slate-100 transition-colors shadow-sm">
                  <lucide-icon name="download" class="w-4 h-4"></lucide-icon>
                  <span class="hidden sm:inline text-xs">Word</span>
                </button>
                <button (click)="exportToPdf()" class="flex items-center gap-2 px-4 py-2 bg-black text-white rounded-lg font-bold hover:bg-slate-800 transition-colors shadow-sm">
          ...
          <!-- Report Paper -->
          <div class="max-w-4xl mx-auto bg-white min-h-screen shadow-sm rounded-2xl p-8 md:p-16 lg:p-20 relative overflow-hidden result-container border border-slate-200">
             
             <!-- Print Header -->
             <div class="print-header mb-8 text-center hidden print:block">
                <h1 class="text-xl font-bold">{{ config().appName }}</h1>
                <p class="text-xs text-gray-500">گزارش سناریو - {{ formData().timeHorizon }}</p>
             </div>
             ...
             <!-- Report Meta -->
             <div class="mb-10 border-b border-slate-100 pb-6 no-print">
                <div class="flex items-center gap-3 mb-4">
                   <span class="px-2 py-1 bg-black text-white rounded text-[9px] font-bold tracking-widest uppercase">Scenario Planning</span>
                   <span class="text-slate-400 text-[10px] font-mono">{{ record()?.dateStr }}</span>
                </div>
                <h1 class="text-2xl md:text-3xl font-bold text-black leading-tight mb-4">{{ formData().issue }}</h1>
                
                <div class="flex flex-wrap gap-4 text-[10px] font-bold text-slate-500">
                   <div class="flex items-center gap-2 bg-slate-50 px-2.5 py-1 rounded border border-slate-100">
                      <lucide-icon name="shield-alert" class="w-3.5 h-3.5"></lucide-icon>
                      <span>وضعیت: {{ formData().status }}</span>
                   </div>
                   <div class="flex items-center gap-2 bg-slate-50 px-2.5 py-1 rounded border border-slate-100">
                      <lucide-icon name="sliders-horizontal" class="w-3.5 h-3.5"></lucide-icon>
                      <span>ریسک: {{ formData().riskTolerance }}</span>
                   </div>
                </div>
             </div>

             <!-- Chart: Bar (If available) -->
             @if (barData()) {
               <div class="mb-10 bg-slate-50 p-6 rounded-xl border border-slate-200 no-print">
                  <h4 class="text-black font-bold mb-5 flex items-center gap-2 text-xs uppercase tracking-wider">
                     <lucide-icon name="bar-chart-2" class="w-4 h-4"></lucide-icon> 
                     {{ barData().title }}
                  </h4>
                  <div class="space-y-4">
                     @for (label of barData().labels; track label; let i = $index) {
                        <div>
                           <div class="flex justify-between text-[10px] font-bold text-slate-500 mb-1 uppercase">
                              <span>{{ label }}</span>
                              <span>{{ barData().data[i] }}%</span>
                           </div>
                           <div class="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                              <div class="h-full rounded-full transition-all duration-1000 ease-out bg-black"
                                   [style.width.%]="barData().data[i]"></div>
                           </div>
                        </div>
                     }
                  </div>
             ...
             <div class="mt-12 pt-6 border-t border-slate-100 flex justify-between items-center no-print opacity-60">
                <div class="flex items-center gap-2">
                   <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Generated by Didban AI Core</span>
                </div>
                <span class="text-[9px] font-mono text-slate-300 tracking-widest">CONFIDENTIAL</span>
             </div>
             
             <div class="print-only mt-8 pt-6 border-t border-slate-300 hidden print:block text-center">
                <p class="text-[10px] text-slate-500 font-mono">{{ config().footerText }}</p>
             </div>
          </div>
  ...
  styles: [`
    ...
    :host ::ng-deep .markdown-content h2::before { content: ''; @apply w-1.5 h-6 bg-orange-500 rounded-full inline-block; }
    ...
    :host ::ng-deep .markdown-content blockquote { @apply border-r-4 border-orange-200 bg-orange-50/50 p-4 rounded-l-xl italic text-slate-600 mb-6 text-sm; }
...
export class ScenarioResultComponent implements OnInit {
  ...
  formData = signal<ScenarioInputs>({} as ScenarioInputs);
  renderedContent = signal('');
  barData = signal<{ type: string; title: string; labels: string[]; data: number[]; } | null>(null);
  ...
  async ngOnInit() {
    ...
    if (id) {
      ...
      if (rec && rec.type === 'scenario') {
        this.record.set(rec);
        this.formData.set(rec.inputs as ScenarioInputs);
  ...
  exportToWord() {
    ...
    this.exportService.exportToWord(inputs.issue, r.result, {
       'حوزه': inputs.domain,
       'وضعیت': inputs.status,
       'ریسک': inputs.riskTolerance
    });
  }
