...
@Component({
  selector: 'app-admin-analysis-settings',
  standalone: true,
        ...
        <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200">
          <div class="flex items-center justify-between mb-8 pb-6 border-b border-slate-50">
            <div>
              <h3 class="text-2xl font-black text-slate-900 flex items-center gap-3">
                ...
                ساختار فرم تحلیل استراتژیک
              </h3>
              <p class="text-xs text-slate-400 font-bold mt-1 uppercase tracking-widest">مدیریت فیلدهای ورودی و منطق فرم</p>
            </div>
            <button (click)="addField()" class="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-xs shadow-xl hover:bg-indigo-600 transition-all flex items-center gap-2 active:scale-95">
          ...
          <div class="space-y-4">
            @for (field of fields(); track field.id; let i = $index) {
              <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100 group hover:border-indigo-200 hover:bg-white transition-all duration-300 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-1 h-full bg-slate-200 group-hover:bg-indigo-500 transition-colors"></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                  ...
                  <div class="md:col-span-10 grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div class="space-y-1.5">
                      ...
                      <input type="text" [(ngModel)]="field.id" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-mono text-[10px] ltr text-left focus:border-indigo-500 outline-none transition-all">
                    </div>
                    <div class="space-y-1.5">
                      <label class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">عنوان نمایشی</label>
                      <input type="text" [(ngModel)]="field.label" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-xs focus:border-indigo-500 outline-none transition-all">
                    </div>
                    <div class="space-y-1.5">
                      <label class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">نوع ورودی</label>
                      <select [(ngModel)]="field.type" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-xs focus:border-indigo-500 outline-none transition-all appearance-none cursor-pointer">
                    ...
                    @if (field.type === 'select') {
                      <div class="md:col-span-3 space-y-1.5">
                        ...
                        <input type="text" [ngModel]="field.options?.join(', ')" (ngModelChange)="updateOptions(field, $event)" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-xs focus:border-indigo-500 outline-none transition-all" placeholder="گزینه ۱, گزینه ۲, ...">
                      </div>
                    }
                    ...
                    <div class="md:col-span-3 flex flex-wrap items-center gap-8 pt-2">
                      <label class="flex items-center gap-3 cursor-pointer group/check">
                        <div class="relative flex items-center">
                          ...
                          <div class="w-10 h-5 bg-slate-200 rounded-full peer peer-checked:bg-indigo-600 transition-all after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:translate-x-5"></div>
                        </div>
                        <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover/check:text-indigo-600 transition-colors">اجباری</span>
                      </label>
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <div class="flex p-1 bg-white border border-slate-200 rounded-lg">
                           <button (click)="field.width = 'half'" [class]="field.width === 'half' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-600'" class="px-3 py-1 rounded-md text-[9px] font-black uppercase transition-all">Half</button>
                           <button (click)="field.width = 'full'" [class]="field.width === 'full' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-600'" class="px-3 py-1 rounded-md text-[9px] font-black uppercase transition-all">Full</button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div class="mt-12 flex justify-between items-center pt-8 border-t border-slate-100">
            ...
            <button (click)="saveChanges()" class="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-2xl shadow-indigo-500/20 hover:bg-indigo-700 transition-all active:scale-95 flex items-center gap-3">
        ...
        <div class="sticky top-24 space-y-6">
          <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-slate-900/20 relative overflow-hidden">
             ...
             <div class="relative z-10">
                <h4 class="text-xs font-black uppercase tracking-[0.3em] text-indigo-400 mb-4">پیش‌نمایش زنده</h4>
                <div class="space-y-6">
                          ...
                          <div [class]="field.width === 'full' ? 'col-span-2' : 'col-span-1'" class="space-y-2">
                             ...
                             @if (field.type === 'textarea') {
                                ...
                                <div class="relative inline-flex items-center cursor-pointer">
                                   ...
                                   <div class="w-10 h-5 bg-white/20 rounded-full peer peer-checked:bg-indigo-400 transition-all after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:translate-x-5"></div>
                                </div>
                             } @else {
                   ...
                   <div class="flex items-center gap-3 text-[10px] font-bold text-slate-400 leading-relaxed">
                      <lucide-icon name="info" class="w-5 h-5 text-indigo-400 shrink-0"></lucide-icon>
                      <span>این یک نمای شماتیک از فرم نهایی است. تغییرات شما بلافاصله در بخش تحلیل استراتژیک اعمال خواهد شد.</span>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div class="bg-indigo-50 rounded-[2rem] p-6 border border-indigo-100">
             <h5 class="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2">نکته امنیتی</h5>
             <p class="text-[11px] text-indigo-800 font-bold leading-relaxed">
...
export class AnalysisSettingsComponent {
  ...
  fields = signal<DynamicField[]>([...this.configService.config().analysisFields]);
  ...
  saveChanges() {
    this.configService.updateConfig({ analysisFields: this.fields() });
    alert('ساختار فرم تحلیل استراتژیک با موفقیت به‌روزرسانی شد.');
  }
}
